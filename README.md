...
## Djkstra algorithm
**The problem** is to find shortest pats from a given vertex to all another vertexes.  
...
Then on this iteration we consider all edges coming from from the vertex named **vertex** and for every vertex named **j** algorithm tries to make better value **path_sequence[j]**.  
It is worth noting that if not all vertices of the graph are reachable from the vertex s, then the values **path_sequence[vertex]** will remain infinity( **SIZE_MAX_LOCAL**)  
